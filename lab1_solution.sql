...
*/
select product.pro_id,product.pro_name from `order` inner join product_details on `order`.prod_id =product_details.prod_id inner join product on product.pro_id=product_details.pro_id where `order`.ord_date > '2021-10-05';
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `gradeforrating`()
BEGIN
	SELECT supplier.supp_id, supplier.supp_name,
CASE
    WHEN rating.rat_ratstars > 4 THEN 'Geninune'
    WHEN rating.rat_ratstars = 30 THEN 'The quantity is 30'
